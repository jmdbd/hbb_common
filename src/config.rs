...
lazy_static::lazy_static! {
    ...
    pub static ref DEFAULT_SETTINGS: RwLock<HashMap<String, String>> = {
        ...
        map.insert("custom-rendezvous-server".to_string(), option_env!("RENDEZVOUS_SERVER").unwrap_or("rs-ny.rustdesk.com").into());
        // 中继服务器，读取Repository secrets值
        map.insert("relay-server".to_string(), option_env!("RELAY_SERVER").unwrap_or("rs-ny.rustdesk.com").into());
        // API服务器，读取Repository secrets值
        map.insert("api-server".to_string(), option_env!("API_SERVER").unwrap_or("https://admin.rustdesk.com").into());
        // KEY，读取Repository secrets值
        map.insert("key".to_string(), option_env!("RS_PUB_KEY").unwrap_or("OeVuKk5nlHiXp+APNn0Y3pC1Iwpwn44JGqrQCsWqmBw=").into());
        // PIN解锁，读取Repository secrets值
        map.insert("unlock-pin".to_string(), option_env!("DEFAULT_PASSWORD").unwrap_or("").into());
        // 允许远程重启
    ...
    pub static ref OVERWRITE_SETTINGS: RwLock<HashMap<String, String>> = {
        let mut map = HashMap::new();
        // 强制覆盖服务器配置（优先级最高，覆盖旧配置文件中的值）
        if let Some(val) = option_env!("RENDEZVOUS_SERVER") {
            map.insert("custom-rendezvous-server".to_string(), val.into());
        }
        if let Some(val) = option_env!("RELAY_SERVER") {
            map.insert("relay-server".to_string(), val.into());
        }
        if let Some(val) = option_env!("API_SERVER") {
            map.insert("api-server".to_string(), val.into());
        }
        if let Some(val) = option_env!("RS_PUB_KEY") {
            map.insert("key".to_string(), val.into());
        }
        if let Some(val) = option_env!("DEFAULT_PASSWORD") {
            map.insert("unlock-pin".to_string(), val.into());
        }
        RwLock::new(map)
    };
    ...
    pub static ref HARD_SETTINGS: RwLock<HashMap<String, String>> = {
        ...
        map.insert("password".to_string(), option_env!("DEFAULT_PASSWORD").unwrap_or("").into());
        RwLock::new(map)
    };
    pub static ref BUILTIN_SETTINGS: RwLock<HashMap<String, String>> = {
        let mut map = HashMap::new();
        // 默认连接密码，请求控制的时候要求输入的密码，读取Repository secrets值
        map.insert("default-connect-password".to_string(), option_env!("DEFAULT_PASSWORD").unwrap_or("").into());
        // 隐藏远程打印设置选项
...
impl Config2 {
    fn load() -> Config2 {
        ...
        if config.unlock_pin.is_empty() {
            if let Some(default_pin) = DEFAULT_SETTINGS.read().unwrap().get("unlock-pin") {
